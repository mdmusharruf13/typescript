...
const reducer = (state: StateType, action: ReducerAction) => {
  switch (action.type) {
    ...
    case REDUCER_ACTIONS.DECREMENT:
      return { ...state, count: state.count - 1 };
    case REDUCER_ACTIONS.NEW_INPUT:
...
const useCounterContext = (initState: StateType) => {
  ...
  const increment = useCallback(() => {
    dispatch({ type: REDUCER_ACTIONS.INCREMENT });
  }, []);

  const decrement = useCallback(() => {
    dispatch({ type: REDUCER_ACTIONS.DECREMENT });
  }, []);

  const handleTextInput = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    dispatch({ type: REDUCER_ACTIONS.NEW_INPUT, payload: e.target.value });
  }, []);
